...
interface ExerciseSetFormProps {
    ...
    onSave: (data: { name: string; series: number; exercises: ExerciseInSetInput[] }) => Promise<void>;
    onCancel: () => void;
}

export function ExerciseSetForm({ sessionId, existingSet, onSave, onCancel }: ExerciseSetFormProps) {
    ...
    const [selectedTemplates, setSelectedTemplates] = useState<{ [key: number]: ExerciseTemplate | undefined }>({});
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <div style={{
                            ...
                            <label className="form-label" style={{ marginBottom: 0 }}>Ejercicios</label>
                            <button
                                type="button"
                                onClick={addExercise}
                                style={{
                                                        ...
                                                        <div key={fieldName} style={{ marginBottom: '0.75rem' }}>
                                                            ...
                                                            {fieldDef.type === 'text' ? (
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => updateExerciseData(index, fieldName, e.target.value)}
                                                                ...
                                                                <input
                                                                    type={fieldDef.type === 'integer' || fieldDef.type === 'float' || fieldDef.type === 'number' ? 'number' : 'text'}
                                                                    className="form-input"
                                                                    value={exercise.data[fieldName] || ''}
                                                                    onChange={(e) => {
                                                                        const value = (fieldDef.type === 'integer' || fieldDef.type === 'float' || fieldDef.type === 'number')
                    ...
                    {error && (
                        <p style={{ color: '#dc3545', marginBottom: '1rem', fontSize: '0.875rem' }}>
                            {error}
                        </p>
                    )}
