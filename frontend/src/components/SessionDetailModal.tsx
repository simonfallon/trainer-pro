...
export function SessionDetailModal({
  ...
  const [activeTab, setActiveTab] = useState<"exercises" | "times" | "sets">("exercises");
  ...
  const lapTimeMeasurements = exercises.filter((ex) => ex.custom_name === "Toma de Tiempo BMX");
  const regularExercises = exercises.filter((ex) => ex.custom_name !== "Toma de Tiempo BMX");

  // Check if trainer is physio to show Circuitos tab
  const isPhysio =
    trainer?.discipline_type?.toLowerCase() === "physio" ||
    trainer?.discipline_type?.toLowerCase() === "fisioterapia";
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        ...
        style={{ ...darkStyles.modal, maxWidth: "600px" }}
      >
        ...
        >
          <button
            ...
            style={{
              ...
              borderBottom:
                activeTab === "exercises" ? `3px solid ${theme.colors.primary}` : "none",
          ...
          {/* Only show Tiempos tab for BMX trainers */}
          {isBMX && (
            <button
              ...
              style={{
                ...
                border: "none",
                backgroundColor: "transparent",
                color: activeTab === "times" ? theme.colors.primary : theme.colors.secondary,
                borderBottom: activeTab === "times" ? `3px solid ${theme.colors.primary}` : "none",
                fontWeight: activeTab === "times" ? 600 : 400,
                cursor: "pointer",
                fontSize: "1rem",
                transition: "all 0.2s",
              }}
          ...
          {/* Only show Circuitos tab for physio trainers */}
          {isPhysio && (
            <button
            ...
            {regularExercises.length > 0 ? (
              <div>
                {regularExercises.map((exercise) => (
                  ...
                  >
                    <div
                      style={{ fontWeight: 600, color: theme.colors.text, marginBottom: "0.5rem" }}
                    >
                      {exercise.custom_name}
                    </div>
                    {exercise.data && Object.keys(exercise.data).length > 0 && (
                      <div style={{ fontSize: "0.875rem", color: theme.colors.secondary }}>
            ...
            {lapTimeMeasurements.length > 0 ? (
              <div>
                {lapTimeMeasurements.map((measurement, idx) => {
                  const lapTimes = measurement.data.lap_times_ms || [];
                  const avgTime =
                    lapTimes.length > 0
                      ? lapTimes.reduce((sum: number, time: number) => sum + time, 0) /
                        lapTimes.length
                      : 0;
                  ...
                  return (
                    <div
                      key={measurement.id}
                      style={{ marginBottom: idx < lapTimeMeasurements.length - 1 ? "1.5rem" : 0 }}
                    >
                      <div
                        style={{
                          fontSize: "0.875rem",
                        ...
                        <div style={{ display: "flex", gap: "1rem" }}>
                          ...
                          <div style={{ flex: 1, overflowX: "auto" }}>
                            ...
                            >
                              <thead>
                                <tr
                                  style={{ borderBottom: `1px solid ${theme.colors.secondary}30` }}
                                >
                                  <th
                                    style={{
                                      padding: "0.5rem",
                              ...
                              <tbody>
                                {lapTimes.map((lapTime: number, lapIdx: number) => (
                                  <tr
                                    key={lapIdx}
                                    style={{
