...
export const StopwatchModal: React.FC<StopwatchModalProps> = ({
  ...
  const handleReset = () => {
    if (laps.length > 0) {
      const confirmed = window.confirm(
        "¿Estás seguro de que quieres reiniciar el cronómetro? Se perderán todos los tiempos de vuelta."
      );
      if (!confirmed) return;
    }
  ...
  const handleLap = () => {
    ...
    const lapTime = laps.length === 0 ? elapsedMs : elapsedMs - laps[laps.length - 1].split_time_ms;
