...
describe("locationUtils", () => {
  describe("getGoogleMapsUrl", () => {
    ...
    it("should use address search when only address is available", () => {
      ...
      const url = getGoogleMapsUrl(location);
      expect(url).toBe(
        "https://www.google.com/maps/search/?api=1&query=Calle%2010%20%2343A-30%2C%20Medell%C3%ADn%2C%20Antioquia"
      );
    });
    ...
    it("should handle missing city and region in address search", () => {
      ...
      const url = getGoogleMapsUrl(location);
      expect(url).toBe(
        "https://www.google.com/maps/search/?api=1&query=Calle%2010%20%2343A-30%2C%20%2C"
      );
    });
    ...
    it("should handle coordinates with many decimal places", () => {
      ...
      const url = getGoogleMapsUrl(location);
      expect(url).toBe(
        "https://www.google.com/maps/search/?api=1&query=6.24370123456789,-75.43716698765432"
      );
    });
  });
});
